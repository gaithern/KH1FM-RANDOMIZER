...
def write_spell_info(settings_file, mp_cost_file):
    ...
    spell_mp_cost_definitions = read_csv(BASE_DIR / "Documentation" / "KH1FM Documentation - Spell MP Cost.csv")
    
    # Handle spell descriptions
    sysmsg_bytes = read_bytes(kh1_data_path / "remastered" / "menu" / "uk" / "sysmsg.bin" / "UK_sysmsg.binl")
    new_sysmsg_bytes = replace_sysmsgs(sysmsg_bytes, spell_mp_costs_data)
    write_bytes(BASE_DIR / "Working" / "remastered" / "menu" / "uk" / "sysmsg.bin" / "UK_sysmsg.binl", bytes(new_sysmsg_bytes))
    ...
    if settings_data["scaling_spell_potency"]:
        spell_effectiveness_definitions = read_csv(BASE_DIR / "Documentation" / "KH1FM Documentation - Spell Effectiveness.csv")
