...
def replace_specific_gummi_item_string(index, description):
    ...
    if description in TOO_LONG_DESCRIPTIONS:
        description = TOO_LONG_DESCRIPTIONS[description]
    gummi_item_description_bytes = read_bytes(kh1_data_path / "exchange" / "UK_gumi_mes_data.bin")
    ...
    write_bytes(BASE_DIR / "Working" / "exchange" / "UK_gumi_mes_data.bin", bytes(new_gummi_item_description_bytes))

def write_gummi_items(settings_file):
    kh1_data_path = BASE_DIR / "Working"
    ...
    for i in range(len(gummi_item_description_bytes)):
        if gummi_item_description_bytes[i] == 0x00:
            offsets.append(i + 1)
    offsets = offsets[:-2]
    gummi_item_description_offset_bytes = read_bytes(kh1_data_path / "exchange" / "UK_gumi_mes_ofs.bin")
    for i in range(len(offsets)):
        gummi_item_description_offset_bytes = replace_value_at_index(gummi_item_description_offset_bytes, i, offsets[i])
    write_bytes(BASE_DIR / "Working" / "exchange" / "UK_gumi_mes_ofs.bin", gummi_item_description_offset_bytes)
    
    
    gummi_items_lua_str = read_file(BASE_DIR / "Template Luas" / "1fmRandoGummiItems.lua")
    gummi_items_lua_str = gummi_items_lua_str.replace("gummi_item_count = 0", f"gummi_item_count = {settings_num}")
    write_file(BASE_DIR / "Working" / "scripts" / "1fmRandoGummiItems.lua", gummi_items_lua_str)