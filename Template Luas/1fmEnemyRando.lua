...
local function AddAddrs()
	...
	local logfile = io.open(SCRIPT_PATH .. "/randofiles/enemyrandolog.txt", "w+")
	for i=0, 32 do
...
function _OnInit()
	if GAME_ID == 0xAF71841E and ENGINE_TYPE == "BACKEND" then
		...
		if canExecute then
			...
			seedfile = io.open(SCRIPT_PATH .. "/randofiles/seed.txt", "r")
			if seedfile ~= nil then
				text = seedfile:read()
				seedstring = text
				seed = tonumber(text) % math.maxinteger
				if seed == nil then
					seed = Djb2(text)
				end
				...
				seedfile = io.open(SCRIPT_PATH .. "/randofiles/seed.txt", "w")
